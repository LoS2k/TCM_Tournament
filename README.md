...
## Головна сторінка (index.html) — нові фішки

- **Відлік до старту** — вкажи дату й час в адмінці (секція "Головна сторінка") і на головній з'явиться живий таймер днів/годин/хвилин/секунд. Коли турнір переходить у статус "live" — таймер автоматично змінюється на "Турнір триває зараз".
- **Найближчий матч** — автоматично підтягується перший матч зі статусом live або upcoming (де обидві команди вже відомі).
- **Топ-3 команди** — міні-рейтинг з медалями, оновлюється разом із результатами.
- **YouTube-канали** — додай коментаторів турніру в адмінці (ім'я + посилання), на головній з'являться вбудовані відеоплеєри. Підтримуються звичайні посилання на відео, `youtu.be`, `/live/`, а також посилання на канал (`/@нікнейм`, `/channel/ID`) — для каналу плеєр покаже останній/поточний контент каналу.
- **Стрічка новин** — пиши короткі оголошення в адмінці (наприклад про перенесення матчу), вони з'являються на головній з міткою часу, найновіші зверху.
- **Discord-кнопка** — встав посилання на сервер в адмінці, кнопка з'явиться у верхній частині головної сторінки.

Усі ці поля необов'язкові — якщо порожні, відповідний блок просто не показується на сторінці.
